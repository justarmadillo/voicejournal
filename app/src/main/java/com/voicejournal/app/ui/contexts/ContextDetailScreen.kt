...
fun ContextDetailScreen(
    ...
    viewModel: ContextDetailViewModel = hiltViewModel()
) {
    ...
    Column(modifier = Modifier.fillMaxSize()) {
        TopAppBar(
            title = { Text(context?.name ?: "Context") },
            navigationIcon = {
            ...
            if (voiceLogs.isEmpty()) {
                item {
                    EmptyState(
                        ...
                        title = "No recordings",
                        subtitle = "Record a voice memo about this context to see it here"
                    )
                }
            }
...
@Composable
private fun ContextTimelineLogCard(
    log: VoiceLog,
    isPlaying: Boolean,
    onPlay: () -> Unit,
    onPause: () -> Unit,
            ...
            Row(
                ...
                Column(modifier = Modifier.weight(1f)) {
                    ...
                    Row(verticalAlignment = Alignment.CenterVertically) {
                        Text(
                            text = "${DateTimeUtil.formatDate(log.createdAt)} - ${DurationUtil.formatDuration(log.durationMs)}",
